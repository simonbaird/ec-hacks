...
function configure_chains() {
    ...
    kubectl patch -n tekton-chains configmap/chains-config --type merge --patch \
        "{\"data\": ${data}}"
    ...
    kubectl -n tekton-chains get configmap chains-config -o yaml | yq '.data'
}
...
function wait_for_pipelinerun() {
  ...
  while :
  do
      value="$(kubectl get pr "${pr_name}" -o yaml | yq '.metadata.annotations."chains.tekton.dev/signed"')"
      if [[ "$value" == "true" ]]; then
          break
      fi
      if [[ "$value" == "false" ]]; then
